...
class DocumentCertification extends Contract {
    ...
    async IssueDocument(ctx, id, organization, docyType, docName, studentName, grade, issueDate, imgHash) {
    ...
    async UpdateDocument(ctx, id, organization, docyType, docName, studentName, grade, issueDate, imgHash) {
        const exists = await this.DocumentExists(ctx, id);
        if (!exists) {
            throw new Error(`The asset ${id} does not exist`);
        }
